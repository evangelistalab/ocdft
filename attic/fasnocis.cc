...
//#include <psi4/libscf_solver/integralfunctors.h>
//#include <psi4/libscf_solver/omegafunctors.h>
#include "helpers.h"
...
FASNOCIS::FASNOCIS(SharedWavefunction ref_scf,
                   std::shared_ptr<SuperFunctional> functional,
                   Options &options, std::shared_ptr<PSIO> psio)
    : UHF(ref_scf, functional, options, psio), do_excitation(false) {}

FASNOCIS::FASNOCIS(std::shared_ptr<SuperFunctional> functional,
                   Options &options, std::shared_ptr<PSIO> psio,
                   SharedWavefunction ref_scf,
                   std::vector<std::pair<size_t, size_t>> vec_frozen_mos,
                   std::vector<size_t> aocc, std::vector<size_t> bocc,
                   Dimension nadoccpi)
    : UHF(ref_scf, functional, options, psio), ref_scf_(ref_scf),
      vec_frozen_mos_(vec_frozen_mos), afocc_(aocc), bfocc_(bocc),
      nadoccpi_(nadoccpi), do_excitation(true) {
...
void FASNOCIS::form_C_active() {
  ...
  for (size_t h = 0; h < nirrep_; ++h) {
    for (size_t p = 0; p < nmopi_[h]; ++p) {
      ...
      if (std::find(vec_frozen_mos_.begin(), vec_frozen_mos_.end(), h_p) ==
          vec_frozen_mos_.end()) {
        vec_active_mos_.push_back(h_p);
        namopi_[h] += 1;
#if DEBUG_NOSCF
        outfile->Printf(
            "\n  Adding MO (h = %zu, mo = %zu) to the vector of active MOs", h,
            p);
#endif
      }
    }
  ...
  Ft_amo_ = SharedMatrix(
      new Matrix("Total Fock matrix in the active MOs", namopi_, namopi_));
...
void FASNOCIS::combine_C_frozen_active() {
  ...
  copy_block(Ca_f_, 1.0, Ca_, 1.0, nsopi_, nfmopi_, zerodim, zerodim, zerodim,
             nadoccpi_);
  copy_block(Cb_f_, 1.0, Cb_, 1.0, nsopi_, nfmopi_, zerodim, zerodim, zerodim,
             nadoccpi_);
#if DEBUG_NOSCF
  Ca_->print();
  Cb_->print();
  ...
  copy_block(Ca_a_, 1.0, Ca_, 1.0, nsopi_, nauoccpi_, zerodim, nadoccpi_,
             zerodim, offset);
  copy_block(Cb_a_, 1.0, Cb_, 1.0, nsopi_, nauoccpi_, zerodim, nadoccpi_,
             zerodim, offset);
#if DEBUG_NOSCF
  Ca_->print();
  Cb_->print();
...
double FASNOCIS::compute_E() {
  ...
  double two_electron_E =
      0.5 * (Da_->vector_dot(Fa_) + Db_->vector_dot(Fb_) - one_electron_E);
