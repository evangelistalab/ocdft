...
void copy_block(SharedMatrix A, double alpha, SharedMatrix B, double beta, Dimension rowspi,
                Dimension colspi, Dimension A_rows_offsetpi, Dimension A_cols_offsetpi,
    ...
    for (int h = 0; h < nirrep_; ++h) {
        ...
        if (nrows * ncols != 0) {
            ...
            for (int i = 0; i < nrows; ++i) {
                for (int j = 0; j < ncols; ++j) {
                    B_h[i + B_row_offset][j + B_col_offset] =
                        alpha * A_h[i + A_row_offset][j + A_col_offset] +
                        beta * B_h[i + B_row_offset][j + B_col_offset];
                }
            }
        }
    }
