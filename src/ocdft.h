...
public:
  explicit UOCDFT(SharedWavefunction ref_scf,
  ...
  explicit UOCDFT(std::shared_ptr<SuperFunctional> functional, Options &options,
                  std::shared_ptr<PSIO> psio, SharedWavefunction ref_scf,
                  int state);
  explicit UOCDFT(std::shared_ptr<SuperFunctional> functional, Options &options,
                  std::shared_ptr<PSIO> psio, SharedWavefunction ref_scf,
                  int state, int symmetry);
  virtual ~UOCDFT();
  ...
  double oscillator_strength_s_plus_x() {
    return oscillator_strength_s_plus_x_;
  }
  double oscillator_strength_s_plus_y() {
    return oscillator_strength_s_plus_y_;
  }
  double oscillator_strength_s_plus_z() {
    return oscillator_strength_s_plus_z_;
  }
...
protected:
  ...
  std::vector<std::tuple<double, int>> hole_subspace(SharedWavefunction wfn,
                                                       SharedMatrix Ca);
  ...
  corresponding_orbitals(SharedMatrix A, SharedMatrix B, Dimension dima,
                         Dimension dimb);
  ...
  std::pair<double, double> matrix_element(SharedDeterminant A,
                                           SharedDeterminant B);
